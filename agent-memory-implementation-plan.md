# Local-First Agent Memory Implementation Plan
...
Markdown files are durable and syncable. Generated data under `.agent-memory/` is local, disposable, and rebuildable.
...
```bash
memory search "vector db" --project foo --type decision --status active
memory search "preferences" --scope user
memory search "agent memory" --include-related
```
...
```bash
memory recall "agent memory" --budget 1500
```
...
```bash
memory brief "What did we decide about Obsidian memory sync?" --budget 1200
```
...
- Rebuild index with `memory reindex`.
...
- `memory reindex` restores local working state.
...
Reason: FTS plus good chunking, graph relations, budget packing, lifecycle handling, and memory brief already gives most of the practical value. Semantic search can be added after schema and lifecycle behavior stabilize.
