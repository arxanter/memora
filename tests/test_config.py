...
from config import (
    ConfigError,
    ENV_AGENT_AUTO_RECALL,
    ENV_AGENT_MEMORY_ENABLED,
    ENV_SEMANTIC_MODEL,
...
def test_load_config_includes_agent_policy_defaults_and_env_overrides(tmp_path, monkeypatch):
    vault = tmp_path / "memory-vault"
    init_vault(vault)
    monkeypatch.setenv(ENV_AGENT_MEMORY_ENABLED, "false")
    monkeypatch.setenv(ENV_AGENT_AUTO_RECALL, "false")

    config = load_config(vault)

    assert config.agent_policy.aliases == ["Remi", "Рэми", "Реми"]
    assert config.agent_policy.enabled is False
    assert config.agent_policy.auto_recall is False
...
def test_load_config_ignores_legacy_recall_policy_yaml_overrides(tmp_path):
    vault = tmp_path / "memory-vault"
    init_vault(vault)
...
""".lstrip(),
    ...
    assert config.recall_policies["default"].include_profile is True
    assert config.recall_policies["review"].include_profile is False
    assert "custom" not in config.recall_policies


def test_load_config_uses_runtime_recall_defaults_for_old_yaml(tmp_path):
    vault = tmp_path / "memory-vault"
    init_vault(vault)
    config_path = vault / "config.yaml"
    config_path.write_text(
...
def test_load_config_ignores_legacy_freshness_yaml_overrides(tmp_path):
    vault = tmp_path / "memory-vault"
    init_vault(vault)
...
index_freshness:
  enabled: false
  refresh_before_search: false
  refresh_before_recall: false
""".lstrip(),
        encoding="utf-8",
    )
    ...
    assert config.index_freshness.refresh_before_recall is True


def test_load_config_keeps_profile_as_runtime_default_not_public_config(tmp_path):
    vault = tmp_path / "memory-vault"
    init_vault(vault)
