...
select_python() {
  ...
  if [ -n "$requested" ]; then
    ...
    fail "Python >= 3.10 is required for the MCP extra, but $requested is Python $(python_version "$requested" 2>/dev/null || printf 'unknown'). Install Python 3.10+ or pass --python /path/to/python3.10."
  fi
  ...
  fail "Python >= 3.10 is required for local MCP installation. Install Python 3.10+ (for example with Homebrew: brew install python@3.12) or rerun with --python /path/to/python3.10."
}
...
if [ "$SKIP_INSTALL" != "1" ]; then
  if [ "$USE_VENV" = "1" ]; then
    ...
    if ! python_ok "$PYTHON_CMD"; then
      fail "managed venv Python is $(python_version "$PYTHON_CMD"), but MCP requires Python >= 3.10"
    fi
    log "upgrading pip"
    run_cmd "$PYTHON_CMD" -m pip install -U pip
  elif ! python_ok "$PYTHON_CMD"; then
    fail "Python >= 3.10 is required for the MCP extra, but $PYTHON_CMD is Python $(python_version "$PYTHON_CMD")"
  fi

  EXTRA_SUFFIX="[mcp]"
  if [ "$WITH_TEST" = "1" ]; then
    EXTRA_SUFFIX="[mcp,test]"
  fi
  log "installing Agent Memory .$EXTRA_SUFFIX"
...
exec "__PYTHON_CMD__" -m agent_memory.mcp_server "$@"
'
...
MCP_WRAPPER="${MCP_WRAPPER//__PYTHON_CMD__/$PYTHON_CMD}"
MCP_WRAPPER="${MCP_WRAPPER//__INSTALL_DIR__/$INSTALL_DIR}"
...
MCP_WRAPPER="${MCP_WRAPPER//__DEFAULT_VAULT_EXPORT__/$DEFAULT_VAULT_EXPORT}"
...
write_file "$BIN_DIR/memory-mcp" 0755 "$MCP_WRAPPER"
...
MCP client config:
{
  "mcpServers": {
    "agent-memory": {
      "command": "$BIN_DIR/memory-mcp",
      "env": {
        "AGENT_MEMORY_VAULT": "${VAULT_PATH:-/path/to/MemoryVault}"
      }
    }
  }
}
...
Notes:
  - MCP is stdio-based; Claude Code/Cursor should launch memory-mcp on demand.
  - The background service is for local health/maintenance hooks, not a stdio MCP daemon.
EOF
