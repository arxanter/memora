...
def _connect_index(config: MemoryConfig) -> sqlite3.Connection:
    if not config.index_file.exists():
        raise RetrievalIndexError(
            f"SQLite index not found at {config.index_file}; run `memory reindex --vault {config.vault_path}` before searching."
        )
    ...
    try:
        ...
        connection.close()
        raise RetrievalIndexError(
            f"SQLite index at {config.index_file} could not be read; run `memory reindex --vault {config.vault_path}`."
        ) from exc
