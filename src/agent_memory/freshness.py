...
from agent_memory.indexer import ReindexResult, reindex_vault
...
def default_freshness_state_path(config: MemoryConfig) -> Path:
    ...
    return config.vault_path / config.agent_memory_dir / "cache" / "freshness-state.json"
...
def iter_freshness_files(config: MemoryConfig) -> tuple[Path, ...]:
    ...
    local_state_root = root / config.agent_memory_dir
...
def refresh_index_if_needed(
    ...
    *,
    ...
    """Run `memory reindex` only when tracked durable files changed."""
