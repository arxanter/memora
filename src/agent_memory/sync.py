...
from agent_memory.schema import iter_memory_markdown_files, parse_markdown_document
...
class VaultLock:
    ...
    This coordinates local processes that agree to use Agent Memory locks. It is
    ...
    def __init__(
        ...
        *,
        ...
        self.path = config.vault_path / config.agent_memory_dir / "locks" / f"{self.name}.lock"
...
def detect_sync_conflicts(config: MemoryConfig) -> ConflictReport:
    ...
    markdown_files = _iter_syncable_markdown_files(config.vault_path, config.agent_memory_dir)
...
def _iter_syncable_markdown_files(vault_path: PathLike, agent_memory_dir: str) -> tuple[Path, ...]:
    root = Path(vault_path)
    local_state_root = root / agent_memory_dir
    return tuple(
        sorted(
            path
